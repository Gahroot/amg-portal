...
def upgrade() -> None:
    ...
    op.create_table(
        "nps_responses",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, default=sa.text("gen_random_uuid()")),
        sa.Column("survey_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("nps_surveys.id", ondelete="CASCADE"), nullable=False),
        sa.Column("client_profile_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("client_profiles.id"), nullable=False),
        ...
        sa.Column("responded_at", sa.DateTime(timezone=True), nullable=False, default=sa.text("now()")),
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, default=sa.text("gen_random_uuid()")),
        sa.Column("survey_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("nps_surveys.id", ondelete="CASCADE"), nullable=False),
        sa.Column("response_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("nps_responses.id", ondelete="CASCADE"), nullable=False),
        sa.Column("client_profile_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("client_profiles.id"), nullable=False),
