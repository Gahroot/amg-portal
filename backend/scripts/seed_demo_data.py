...
async def get_or_create(
    ...
    **kwargs: Any,
    ...
    if instance:
        logger.info("  Found existing %s: %s", model.__name__, kwargs)
...
async def seed_users(db: AsyncSession, pw_hash: str) -> None:
    ...
    users = [
        (MARCUS_ID, "marcus.wellington@amg.com", "Marcus Wellington", "managing_director"),
        (SARAH_ID, "sarah.blackwood@amg.com", "Sarah Blackwood", "relationship_manager"),
        (JAMES_ID, "james.chen@amg.com", "James Chen", "relationship_manager"),
        (ELENA_ID, "elena.vasquez@amg.com", "Elena Vasquez", "coordinator"),
        (DAVID_ID, "david.park@amg.com", "David Park", "coordinator"),
        (OLIVIA_ID, "olivia.grant@amg.com", "Olivia Grant", "finance_compliance"),
        (PHILIPPE_ID, "philippe.beaumont@amg.com", "Philippe Beaumont", "client"),
        (DIANA_ID, "diana.northcott@amg.com", "Diana Northcott", "client"),
        (ROBERT_ID, "robert.tanaka@amg.com", "Robert Tanaka", "client"),
        (CATHERINE_ID, "catherine.mercer@amg.com", "Catherine Mercer", "partner"),
        (ALEXANDER_ID, "alexander.volkov@amg.com", "Alexander Volkov", "partner"),
        (STEFAN_ID, "stefan.brandt@amg.com", "Stefan Brandt", "partner"),
    ]
...
async def seed_clients(db: AsyncSession) -> None:
    ...
    clients = [
        (CLIENT_BEAUMONT_ID, "Beaumont Family Office", "family_office", SARAH_ID),
        (CLIENT_NORTHCOTT_ID, "Northcott Global Enterprises", "global_executive", SARAH_ID),
        (CLIENT_TANAKA_ID, "Tanaka Holdings", "uhnw_individual", JAMES_ID),
    ]
    ...
    programs = [
        {
            ...
            "objectives": ("Comprehensive estate planning across multi-jurisdictional holdings"),
            "scope": ("Tax optimization, trust restructuring, succession planning"),
        ...
        },
        {
            ...
            "objectives": ("Facilitate seamless executive relocation to Singapore"),
            "scope": ("Immigration, housing, schooling, tax residency"),
        ...
        },
        {
            ...
            "objectives": ("Annual portfolio review and rebalancing"),
            "scope": ("Equity, fixed income, alternatives, real estate"),
        ...
        },
        {
            ...
            "objectives": ("Acquire and curate contemporary art collection"),
            "scope": ("Sourcing, authentication, insurance, storage"),
...
async def seed_milestones(db: AsyncSession) -> None:
    ...
    milestones = [
        ...
        (MS[1], PROG_ESTATE_ID, "Jurisdictional Analysis", "completed", date(2026, 2, 15), 0),
        (MS[2], PROG_ESTATE_ID, "Trust Structure Design", "in_progress", date(2026, 4, 1), 1),
        (MS[3], PROG_ESTATE_ID, "Tax Optimization Review", "at_risk", date(2026, 4, 15), 2),
        (MS[4], PROG_ESTATE_ID, "Succession Plan Drafting", "pending", date(2026, 6, 1), 3),
        # Relocation - 3 milestones
        (MS[5], PROG_RELOCATION_ID, "Immigration Assessment", "pending", date(2026, 5, 1), 0),
        (MS[6], PROG_RELOCATION_ID, "Housing Search", "pending", date(2026, 6, 1), 1),
        (MS[7], PROG_RELOCATION_ID, "School Enrollment", "pending", date(2026, 7, 1), 2),
        # Portfolio Review - 5 milestones (all completed)
        (MS[8], PROG_PORTFOLIO_ID, "Portfolio Audit", "completed", date(2025, 10, 31), 0),
        (MS[9], PROG_PORTFOLIO_ID, "Risk Assessment", "completed", date(2025, 11, 30), 1),
        (MS[10], PROG_PORTFOLIO_ID, "Rebalancing Strategy", "completed", date(2025, 12, 31), 2),
        (MS[11], PROG_PORTFOLIO_ID, "Implementation", "completed", date(2026, 1, 31), 3),
        (MS[12], PROG_PORTFOLIO_ID, "Final Report", "completed", date(2026, 2, 28), 4),
        # Art Collection - 3 milestones
        (MS[13], PROG_ART_ID, "Curator Selection", "in_progress", date(2026, 3, 15), 0),
        (MS[14], PROG_ART_ID, "Artwork Sourcing", "pending", date(2026, 5, 1), 1),
        (MS[15], PROG_ART_ID, "Authentication & Insurance", "pending", date(2026, 7, 1), 2),
    ]
...
async def seed_tasks(db: AsyncSession) -> None:
    ...
    tasks = [
        ...
        (1, MS[1], "Review Swiss holdings", "done", "high", date(2026, 2, 1), ELENA_ID, 0),
        (2, MS[1], "Review UK holdings", "done", "high", date(2026, 2, 5), ELENA_ID, 1),
        (3, MS[1], "Review French holdings", "done", "medium", date(2026, 2, 10), DAVID_ID, 2),
        # Estate Plan - MS2 (in_progress)
        (
            4,
            MS[2],
            "Draft trust deed template",
            "in_progress",
            "high",
            date(2026, 3, 20),
            ELENA_ID,
        ...
        (5, MS[2], "Identify trustee candidates", "todo", "medium", date(2026, 3, 25), DAVID_ID, 1),
        (
            6,
            MS[2],
            "Legal review of trust structures",
            "blocked",
            "urgent",
            date(2026, 3, 30),
            ELENA_ID,
        ...
        (8, MS[3], "Model tax scenarios", "todo", "high", date(2026, 4, 1), ELENA_ID, 1),
        (9, MS[3], "Prepare optimization memo", "todo", "medium", date(2026, 4, 10), DAVID_ID, 2),
        # Estate Plan - MS4 (pending)
        (10, MS[4], "Interview family members", "todo", "medium", date(2026, 5, 1), SARAH_ID, 0),
        (11, MS[4], "Draft succession framework", "todo", "high", date(2026, 5, 15), ELENA_ID, 1),
        (12, MS[4], "Legal validation", "todo", "medium", date(2026, 5, 25), DAVID_ID, 2),
        # Relocation - MS5
        (13, MS[5], "Gather visa requirements", "todo", "high", date(2026, 4, 15), DAVID_ID, 0),
        (14, MS[5], "Submit EP application", "todo", "urgent", date(2026, 4, 25), ELENA_ID, 1),
        (15, MS[5], "Tax residency analysis", "todo", "medium", date(2026, 5, 1), DAVID_ID, 2),
        # Relocation - MS6
        (16, MS[6], "Identify property options", "todo", "medium", date(2026, 5, 15), ELENA_ID, 0),
        (17, MS[6], "Schedule viewings", "todo", "low", date(2026, 5, 25), DAVID_ID, 1),
        (18, MS[6], "Negotiate lease terms", "todo", "medium", date(2026, 6, 1), ELENA_ID, 2),
        # Relocation - MS7
        (
            19,
            MS[7],
        ...
        (20, MS[7], "Prepare applications", "todo", "high", date(2026, 6, 15), ELENA_ID, 1),
        # Portfolio Review - MS8 (completed)
        (21, MS[8], "Collect account statements", "done", "high", date(2025, 10, 15), DAVID_ID, 0),
        (22, MS[8], "Categorize holdings", "done", "medium", date(2025, 10, 25), ELENA_ID, 1),
        # Portfolio Review - MS9 (completed)
        (23, MS[9], "Run risk models", "done", "high", date(2025, 11, 15), ELENA_ID, 0),
        (24, MS[9], "Stress test portfolio", "done", "high", date(2025, 11, 25), DAVID_ID, 1),
        (
            25,
            MS[9],
        ...
        (27, MS[10], "Model tax implications", "done", "medium", date(2025, 12, 25), DAVID_ID, 1),
        (28, MS[10], "Get client sign-off", "done", "urgent", date(2025, 12, 31), JAMES_ID, 2),
        # Portfolio Review - MS11 (completed)
        (29, MS[11], "Execute equity trades", "done", "urgent", date(2026, 1, 15), ELENA_ID, 0),
        (30, MS[11], "Execute fixed income trades", "done", "high", date(2026, 1, 20), DAVID_ID, 1),
        (
            31,
            MS[11],
            "Update alternative allocations",
            "done",
            "medium",
            date(2026, 1, 31),
            ELENA_ID,
        ...
        (32, MS[12], "Draft performance summary", "done", "high", date(2026, 2, 15), DAVID_ID, 0),
        (33, MS[12], "Compile final report", "done", "high", date(2026, 2, 25), ELENA_ID, 1),
        (34, MS[12], "Client presentation", "done", "urgent", date(2026, 2, 28), JAMES_ID, 2),
        # Art Collection - MS13 (in_progress)
        (
            35,
            MS[13],
            "Research curator candidates",
            "in_progress",
            "high",
            date(2026, 3, 10),
            ELENA_ID,
        ...
        (36, MS[13], "Interview shortlist", "todo", "medium", date(2026, 3, 15), DAVID_ID, 1),
        # Art Collection - MS14 (pending)
        (37, MS[14], "Identify target artworks", "todo", "medium", date(2026, 4, 15), ELENA_ID, 0),
        (38, MS[14], "Attend auctions", "todo", "high", date(2026, 4, 30), DAVID_ID, 1),
        # Art Collection - MS15 (pending)
        (
            39,
            MS[15],
            "Engage authentication experts",
            "todo",
            "medium",
            date(2026, 6, 1),
            ELENA_ID,
        ...
        (40, MS[15], "Arrange insurance coverage", "todo", "high", date(2026, 6, 30), DAVID_ID, 1),
    ]
...
async def seed_assignments(db: AsyncSession) -> None:
    ...
    assignments = [
        ...
        },
        {
            ...
            "brief": ("Analyze cross-border tax implications and propose optimization strategies."),
            "status": "accepted",
        ...
        },
        {
            ...
            "brief": (
                "Provide recommendations for portfolio rebalancing based on market conditions."
            ),
            "status": "completed",
        ...
        },
        {
            ...
            "brief": (
                "Review portfolio compliance with regulatory requirements across jurisdictions."
            ),
            "status": "completed",
        ...
        },
        {
            ...
            "brief": (
                "Provide initial valuation and acquisition strategy for contemporary art pieces."
            ),
            "status": "accepted",
...
async def seed_conversations(db: AsyncSession) -> None:
    ...
    convs = [
        ...
        },
        {
            ...
            "title": ("Estate Legal Review — Fortress Legal"),
...
async def seed_communications(db: AsyncSession) -> None:
    ...
    comms = [
        ...
        # Conv 2 - Coordinator <-> Partner (3 messages)
        {
            ...
            "read_receipts": {
                str(ALEXANDER_ID): {
                    "read_at": _ts(NOW - timedelta(hours=12)),
                },
            },
        ...
        },
        {
            ...
            "read_receipts": {
                str(ELENA_ID): {
                    "read_at": _ts(NOW - timedelta(hours=8)),
                },
            },
        ...
        },
        {
            ...
            "body": ("Received, thank you. I will review and provide feedback within 48 hours."),
            "partner_id": PARTNER_FORTRESS_ID,
            "program_id": PROG_ESTATE_ID,
            "read_receipts": {},
        ...
        },
        {
            ...
            "read_receipts": {
                str(SARAH_ID): {
                    "read_at": _ts(NOW - timedelta(hours=20)),
                },
                str(DAVID_ID): {
                    "read_at": _ts(NOW - timedelta(hours=20)),
                },
        ...
        },
        {
            ...
            "body": ("Wednesday 10am works for me. I will prepare the tax filing status update."),
            "program_id": PROG_ESTATE_ID,
            "read_receipts": {},
        ...
        # Conv 4 - RM <-> Client Tanaka (3 messages)
        {
            ...
            "read_receipts": {
                str(ROBERT_ID): {
                    "read_at": _ts(NOW - timedelta(days=2)),
                },
            },
...
async def seed_documents(db: AsyncSession) -> None:
    ...
    docs = [
        ...
        },
        {
            ...
            "entity_id": CLIENT_BEAUMONT_ID,
            "category": "compliance",
            "description": ("Philippe Beaumont — Proof of Address"),
            "uploaded_by": SARAH_ID,
        },
        {
            "id": DC[3],
        ...
        },
        {
            ...
            "entity_id": CLIENT_NORTHCOTT_ID,
            "category": "compliance",
            "description": ("Diana Northcott — Utility Bill (Proof of Address)"),
            "uploaded_by": SARAH_ID,
        },
        {
            "id": DC[5],
        ...
        {
            "id": DC[6],
            "file_path": (f"{base}/tanaka/residence_certificate.pdf"),
            ...
            "entity_id": CLIENT_TANAKA_ID,
            "category": "compliance",
            "description": ("Robert Tanaka — Residence Certificate (Proof of Address)"),
            "uploaded_by": JAMES_ID,
        },
    ]
    for d in docs:
    ...
    for row in notifs:
        ...
        await get_or_create(
            ...
            defaults={
                ...
                "priority": "medium",
                "is_read": is_read,
                "read_at": ((NOW - timedelta(hours=4)) if is_read else None),
            },
            id=NF[idx],
        )
...
async def seed_escalations(db: AsyncSession) -> None:
    ...
    escalations = [
        # Open - task level
        {
            ...
            "description": ("Task has been blocked for 5 days waiting on partner input."),
            "entity_type": "task",
            "entity_id": str(TK[6]),
            "owner_id": ELENA_ID,
            "program_id": PROG_ESTATE_ID,
        ...
        # Acknowledged - milestone level
        {
            ...
            "description": ("Tax optimization milestone is at risk due to delayed tax filings."),
        ...
        # Resolved - program level
        {
            ...
            "level": "program",
            "status": "resolved",
            "title": ("Portfolio Review — Delayed Rebalancing"),
            "description": ("Rebalancing implementation was delayed due to market volatility."),
            "entity_type": "program",
            ...
            "resolution_notes": (
                "Market conditions stabilized. Rebalancing executed successfully."
            ),
        },
    ]
...
async def seed_decision_requests(db: AsyncSession) -> None:
    ...
    decisions = [
            ...
            "options": [
                {
                    ...
                    "description": ("Maximum flexibility for trustees"),
                },
                {
                    "id": "opt2",
                    "label": "Fixed Trust",
                    "description": ("Pre-determined beneficiary shares"),
                },
                {
                    "id": "opt3",
                    "label": "Hybrid Structure",
                    "description": ("Combination approach with reserved powers"),
                },
            ],
            "deadline_date": date(2026, 3, 25),
            "consequence_text": (
                "Without selection, we will proceed with the hybrid structure as recommended."
            ),
            "status": "pending",
            "created_by": SARAH_ID,
        },
        ...
        # Expired
        {
            ...
            "prompt": ("Please confirm your preferred relocation timeline for the Singapore move."),
            "response_type": "choice",
            "options": [
                {
                    "id": "opt1",
            ...
            "consequence_text": ("We will plan for Q3 2026 if no preference is indicated."),
            "status": "expired",
            "created_by": SARAH_ID,
        },
    ]
...
async def seed_program_closure(db: AsyncSession) -> None:
    ...
    await get_or_create(
        ...
        defaults={
            ...
            "status": "completed",
            "checklist": checklist,
            "notes": ("All deliverables approved and client sign-off received."),
            "debrief_notes": (
    ...
    ratings = [
        {
            ...
            "overall_score": 5,
            "comments": ("Exceptional advisory work. Recommendations well-researched."),
        },
        {
            "id": RT[2],
            "program_id": PROG_PORTFOLIO_ID,
            ...
            "overall_score": 4,
            "comments": ("Solid compliance review. Thorough and well-documented."),
        },
    ]
    for r in ratings:
        rid = r.pop("id")
...
async def seed_nps(db: AsyncSession) -> None:
    ...
    await get_or_create(
        ...
        defaults={
            "id": NPS_SURVEY_ID,
            "description": ("Quarterly client satisfaction survey for Q1 2026"),
            "quarter": 1,
            "year": 2026,
            "status": "active",
            "questions": {
                "nps_question": ("How likely are you to recommend AMG to a colleague or peer?"),
                "custom_questions": [
                    {
                        "id": "q1",
                        "text": ("How would you rate your RM's responsiveness?"),
                        "type": "rating",
                    },
                    {
                        "id": "q2",
    ...
    responses = [
        # Promoter - Beaumont
        {
            ...
            "comment": ("Excellent service. Sarah and the team have been outstanding."),
            "custom_responses": {
                "q1": 5,
        ...
        # Passive - Northcott
        {
            ...
            "comment": ("Good service overall, but onboarding could be smoother."),
            "custom_responses": {
                "q1": 4,
        ...
        # Detractor - Tanaka
        {
            ...
            "comment": ("Communication delays during portfolio review were frustrating."),
            "custom_responses": {
                "q1": 2,
...
async def seed_budget_approval(db: AsyncSession) -> None:
    ...
    await get_or_create(
        ...
        defaults={
            "id": CHAIN_ID,
            "description": ("Standard two-step budget approval: Finance then MD"),
            "is_active": True,
            "created_by": MARCUS_ID,
        },
    ...
    await get_or_create(
        ...
        defaults={
            "id": THRESHOLD_ID,
            "description": ("Requests above 50k require standard approval chain"),
    ...
    await get_or_create(
        ...
        defaults={
            ...
            "description": (
                "Engagement of external risk analytics vendor for portfolio stress testing."
            ),
            ...
            "final_comments": ("Approved. Cost justified by portfolio complexity."),
        },
...
async def seed_certificates(db: AsyncSession) -> None:
    ...
    await get_or_create(
        ...
        defaults={
            "id": CERT_TEMPLATE_ID,
            "description": ("Standard program completion clearance certificate"),
    ...
    await get_or_create(
        ...
        defaults={
            ...
            "program_id": PROG_PORTFOLIO_ID,
            "client_id": CLIENT_TANAKA_ID,
            "title": ("Tanaka Portfolio Review — Clearance Certificate"),
            "content": issued_content,
            "populated_data": {
                "program_title": ("Tanaka Investment Portfolio Review"),
    ...
    await get_or_create(
        ...
        defaults={
            ...
            "program_id": PROG_ESTATE_ID,
            "client_id": CLIENT_BEAUMONT_ID,
            "title": ("Beaumont Estate Plan — Draft Clearance Certificate"),
    ...
    await get_or_create(
        ...
        defaults={
            ...
            "notes": ("Certificate issued after compliance review."),
        },
        id=CH[1],
    )
    await get_or_create(
        ...
        defaults={
            ...
            "actor_id": OLIVIA_ID,
            "actor_name": "Olivia Grant",
            "notes": ("Draft certificate created for ongoing program."),
        },
        id=CH[2],
    )
...
async def seed_access_audit(db: AsyncSession) -> None:
    ...
    await get_or_create(
        ...
        defaults={
            ...
            "recommendation": ("Remove access to financial reconciliation module."),
            "status": "open",
        },
        id=FN[1],
    )
    ...
    await get_or_create(
        ...
        defaults={
            ...
            "severity": "low",
            "description": (
                "Partner Stefan Brandt had not logged in for 45 days at time of audit."
            ),
            "recommendation": (
                "Confirm partner engagement status. Consider temporary access suspension."
            ),
            "status": "remediated",
            ...
            "remediation_notes": (
                "Partner confirmed active. Login inactivity due to project gap. Access maintained."
            ),
        },
        id=FN[2],
    )
    ...
    await get_or_create(
        ...
        defaults={
            ...
            "findings": [
                {
                    "type": "strength",
                    "description": ("Strong investment advisory capabilities"),
                    "severity": "positive",
                ...
                },
                {
                    "type": "improvement",
                    "description": (
                        "Tax planning certifications should be updated for 2026 regulatory changes"
                    ),
                    "severity": "medium",
                    "recommendation": ("Complete updated tax planning certification by Q3 2026"),
                },
            ],
            "notes": (
            ...
            "recommendations": ("Renew engagement. Request updated tax planning certification."),
        },
        partner_id=PARTNER_MERIDIAN_ID,
        review_year=2026,
    )
...
async def seed_phase2_meetings(db: AsyncSession) -> None:
    ...
    mt_result = await db.execute(sa_select(MeetingType.id, MeetingType.slug))
