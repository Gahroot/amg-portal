...
def _build_certification_response(c: PartnerCertification, today: date) -> CertificationResponse:
    is_expired = c.expiry_date is not None and c.expiry_date < today
    is_expiring_soon = c.expiry_date is not None and today <= c.expiry_date <= today + timedelta(
        days=30
    )
