"""Escalation workflow endpoints — create, update, status-transition, rules, and playbooks."""
...
async def _build_escalation_response(
    ...
    *,
    response_deadline: object = None,
    is_overdue: object = None,
) -> EscalationResponse:
    owner_result, triggerer_result = await asyncio.gather(
        db.execute(select(User).where(User.id == escalation.owner_id)),
        db.execute(select(User).where(User.id == escalation.triggered_by)),
    )
    ...
    if response_deadline is not None:
        kwargs["response_deadline"] = response_deadline
    if is_overdue is not None:
        kwargs["is_overdue"] = is_overdue
    return EscalationResponse(**kwargs)
...
async def create_manual_escalation(
    ...
    return await _build_escalation_response(db, escalation)
...
async def reassign_escalation_endpoint(
    ...
    return await _build_escalation_response(
        db,
        escalation,
